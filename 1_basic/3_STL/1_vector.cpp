/*
训练 间谍
题目描述X国的情报委员收到一份可靠的信息，信息表明Y国将派间谍去窃取X国的机密文件。
X国指挥官手中有两份名单列表，一份是Y国派往X国的间谍名单列表，另一份是X国以前派往Y国的间谍名单列表。
这两份名单列表可能有些重叠。因为间谍可能同时扮演两个角色，称之为“双重间谍”。
因此，Y国可以把双重间谍送回X国。很明显，这对X国是有利的，因为双重间谍可以把Y国的机密文件带回，而不必担心被Y国边境拘留。
所以指挥官决定抓住由Y国派出的间谍，让普通人和双重间谍进入。那么你能确定指挥官需要抓捕的间谍名单吗？
输入：有几个测试用例。每个测试用例都包含4部分。第1部分包含3个正整数A、B、C，A是进入边境的人数，B是Y国将派出的间谍人数，
C是X国以前派到Y国的间谍人数。第2部分包含A个字符串，为进入边境的人员名单。
...
int main(void)
{
    ...
    while (cin >> a >> b >> c) {
        ...
        for (int i = 0; i < b; i++) { //判断第二行中出现在第一行但没有出现在第三行的字符串
